...
comment on column V_CLIENTE is
'Nombre del cliente.';
...
comment on column VIEW_6.HORARIO_C is
'Horarios de personal solicitados en el contrato.';

comment on column VIEW_6.TIEMPO_C is
...
comment on column V_CONT.ID_PRO is
'Identificador del proveedor.';

comment on column V_CONT.NOMBRE_I is
'Nombre del implemento.';

comment on column V_CONT.PRECIO_UNITARIO_I is
'Precio unitario del implemento de seguridad.';

comment on column V_CONT.CANTIDAD is
'Catidad actual total del implemento.';

comment on column V_CONT.DESCRIPCION_I is
'Descripcion general del implemento.';

comment on column V_CONT.ESTADO_I is
